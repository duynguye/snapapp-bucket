...
interface IDropdownWrapperProps {
  ...
  onKeyPress: (e: KeyboardEvent<HTMLDivElement>) => void | undefined;
 ...
 */
const DropdownWrapper = ({ children, onClick, onBlur, onFocus, onKeyPress, onKeyUp, disabled, active = false }: IDropdownWrapperProps) => {
  if (disabled) {
    return (
    ...
    return (
      <div className={classnames(styles.wrapper, active && styles.wrapperActive)} onClick={onClick} onBlur={onBlur} onFocus={onFocus} onKeyPress={onKeyPress} onKeyUp={onKeyUp} tabIndex={0}>
        {children}
...
class Dropdown extends Component<IDropdownProps> {
  ...
  handleKeyboard = (e: KeyboardEvent<HTMLDivElement>) => {
    const { active } = this.state;
